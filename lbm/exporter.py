...
# Handles data output. It converts the simulation state into .vtk files (Unstructured Grid) for visualization in software like Paraview.
...
class Exporter:
    ...
    def __cache_grid(self):
        if len(self.n) == 2:
            points = np.meshgrid(np.arange(self.n[0] + 1), np.arange(self.n[1] + 1), indexing="ij")
            cell0 = np.meshgrid(np.arange(self.n[0]), np.arange(self.n[1]), indexing="ij")
        elif len(self.n) == 3:
            points = np.meshgrid(np.arange(self.n[0] + 1), np.arange(self.n[1] + 1), np.arange(self.n[2] + 1), indexing="ij")
            cell0 = np.meshgrid(np.arange(self.n[0]), np.arange(self.n[1]), np.arange(self.n[2]), indexing="ij")
        else:
